...
\begin{multicols*}{3}
    ...
    \item Grösster Exponent: $e_{\max} = B^{\text{Stellen von e}} - 1 - bias$
    \item Kleinster Exponent: $e_{\min} = -$bias
    \item Kleinste positive Zahl: \(x_{\min} = B^{e_{\min}-1}\)
    \item Grösste Zahl: \(x_{\max} = (1 - B^{-n}) \cdot B^{e_{\max}}\)
    \item Kleinste Zahl: \(x_{\text{denormalisiert}} = B^{e_{\min} - n + 1}\)
    \item Anzahl der Maschinenzahlen (mit 0): $|M| = 2 \cdot (B - 1) \cdot B^{n + l - 1} + 1$
	\end{itemize}
  ...
  \begin{itemize}
    ...
    \item a-posteriori-Fehlerabschätzung:
      ...
      \end{equation*}
    \item Umformung: $\left | \frac{F(x) - F(y)}{x - y} \right | \leq \alpha$ 
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Betrag: \(|z| = r = \sqrt{x^2 + y^2} = \sqrt{z \cdot z^*}\)
  ...
  \begin{itemize}
    \item $\lambda$ ist Eigenwert von A \(\Leftrightarrow \det(A - \lambda I_n) = 0\)
    \item Charakteristisches Polynom: \(P_A(\lambda) = \det(A - \lambda I_n)\)
    \item Eigenwerte: Nullstellen des charakteristischen Polynoms
    \item Eigenvektoren: Lösen von \((A - \lambda I_n) v = 0\) für jeden Eigenwert \(\lambda\)
    \item $\lambda^{-1}$ ist Eigenwert von \(A^{-1}\) mit demselben Eigenvektor \(v\), falls \(A\) invertierbar ist
    \item Das Spektrum von \(A\) ist die Menge aller Eigenwerte: \(\sigma(A) = \{\lambda_1, \lambda_2, \ldots, \lambda_k\}\)
  \end{itemize}
  \subsection*{Eigenräume}
  \begin{itemize}
    \item Eigenraum zu \(\lambda\): \(E_{\lambda} = \{v \in \mathbb{R}^n | A v = \lambda v\}\)
    \item Dimension des Eigenraums: geometrische Vielfachheit
    \item Vielfachheit eines Eigenwerts als Nullstelle des charakteristischen Polynoms: algebraische Vielfachheit
    \item \(1 \leq \text{geometrische Vielfachheit} \leq \text{algebraische Vielfachheit}\)
    \item Algebraische Vielfachheit 2 $\Rightarrow$ Determinante = 0 $\Rightarrow \varDelta = b^2 + 4ac$
  \end{itemize}
  \subsection*{Diagonalisierbarkeit / Ähnlichkeit}
  \begin{itemize}
    \item Es seien \(A, B \in \mathbb{R}^{n \times n}\). \(A\) und \(B\) heißen ähnlich, wenn eine invertierbare Matrix \(S \in \mathbb{R}^{n \times n}\) existiert, so dass gilt:
      \begin{equation*}
        B = S^{-1} A S
      \end{equation*}
    \item \(A\) ist diagonalisierbar, wenn \(S = D\) eine Diagonalmatrix ist
    \item \(D\) enthält die Eigenwerte von \(A\) auf der Diagonale
    \item \(S\) enthält die zugehörigen Eigenvektoren als Spalten
    \item Wenn $A, B$ ähnlich sind, gilt:
      \begin{itemize}
        \item A und B haben dieselben Eigenwerte mit derselben algebraischen Vielfachheit
        \item Die Eigenvektoren von A und B hängen über die Matrix S zusammen
      \end{itemize}
  \end{itemize}
  \subsection*{von-Mises-Iteration}
  \begin{itemize}
    \item Iterationsvorschrift: \(v^{(k+1)} = \frac{A v^{(k)}}{\|A v^{(k)}\|}\)
    \item Konvergenz gegen den Eigenvektor zum betragsmäßig größten Eigenwert
    \item Approximierter Eigenwert: \(\lambda^{(k)} = \frac{(v^{(k)})^T A v^{(k)}}{(v^{(k)})^T v^{(k)}}\)
  \end{itemize}
  \pagebreak
...
\begin{equation*}
    (x^n)' = n \cdot x^{n-1}
\end{equation*}
\textbf{Ableitung der Exponentialfunktion}
\begin{equation*}
    (e^x)' = e^x
\end{equation*}
...
\begin{equation*}
    (\cos(x))' = -\sin(x)
\end{equation*}
\textbf{Ableitung der Tangensfunktion}
\begin{equation*}
    (\tan(x))' = \frac{1}{\cos^2(x)}
\end{equation*}

\textbf{Ableitung der Umkehrfunktionen}
\begin{itemize}
    \item $(\arcsin(x))' = \frac{1}{\sqrt{1-x^2}}$
    \item $(\arccos(x))' = \frac{-1}{\sqrt{1-x^2}}$
    \item $(\arctan(x))' = \frac{1}{1+x^2}$
    \item Generell gilt: $(f^{-1}(y))' = \frac{1}{f^(f^{-1}(y))}$
\end{itemize}


\subsection*{Grenzwertregel von Bernoulli-de l'Hôpital}
\begin{itemize}
  \item Die Grenzwertregel von Bernoulli-de l'Hôpital ist ein Verfahren zur Bestimmung von Grenzwerten, die in der Form $\frac{0}{0}$ oder $\frac{\infty}{\infty}$ vorliegen.
    \begin{equation*}
        \lim_{x \to x_0} \frac{f(x)}{g(x)} = \lim_{x \to x_0} \frac{f'(x)}{g'(x)}
    \end{equation*}
  \item Falls der Grenzwert $\lim_{x \to x_0} \frac{f(x)}{g(x)}$ in der Form $0 \cdot \infty$ vorliegt, kann die Regel wie folgt umgeformt werden:
    \begin{equation*}
        \lim_{x \to x_0} f(x) \cdot g(x) = \lim_{x \to x_0} \frac{f(x)}{\frac{1}{g(x)}}
    \end{equation*}
  \item Falls der Grenzwert in der Form $\infty - \infty$ vorliegt, kann die Regel wie folgt umgeformt werden:
    \begin{equation*}
        \lim_{x \to x_0} f(x) - g(x) = \lim_{x \to x_0} \frac{f(x) \cdot g(x)}{g(x)}
    \end{equation*}
\end{itemize}

\subsection*{Basiswechsel}
\begin{itemize}
    \item Umwandlung Dezimal in ein anderes Zahlensystem mit Basis B:
    \begin{itemize}
        \item Ganzzahliger Teil: wiederholte Division durch B, Reste notieren (von unten nach oben lesen)
        \item Dezimalteil: wiederholte Multiplikation mit B, ganzzahlige Teile notieren (von oben nach unten lesen)
        \item Beispiel: Dezimal 13.625 in Binär
        \begin{itemize}
            \item Ganzzahliger Teil: \(13 \div 2 = 6\) Rest 1, \(6 \div 2 = 3\) Rest 0, \(3 \div 2 = 1\) Rest 1, \(1 \div 2 = 0\) Rest 1 $\rightarrow$ 1101
            \item Dezimalteil: \(0.625 \times 2 = 1.25\) Ganzzahliger Teil 1, \(0.25 \times 2 = 0.5\) Ganzzahliger Teil 0, \(0.5 \times 2 = 1.0\) Ganzzahliger Teil 1 $\rightarrow$ 101
            \item Ergebnis: \(13.625_{10} = 1101.101_2\)
        \end{itemize}
    \end{itemize}
    \item Umwandlung von Basis B in Dezimal:
    \begin{itemize}
        \item Ganzzahliger Teil: \(Z = \sum_{i=0}^{n} z_i \cdot B^i\)
        \item Dezimalteil: \(D = \sum_{i=1}^{m} z_{-i} \cdot B^{-i}\)
    \end{itemize}
\end{itemize}
\end{multicols*}
\end{document}
