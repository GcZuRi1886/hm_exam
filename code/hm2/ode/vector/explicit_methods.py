...
def euler_method_vector(
    f: Callable[[float, np.ndarray], np.ndarray],
    x0: float,
    z0: np.ndarray,
    step_control: StepSizeControl,
) -> tuple[np.ndarray, np.ndarray]:
    """Implements the Euler method for solving vectorial ODE systems.
    Args:
    ...
    for i in range(step_control.n):
        z_values[:, i + 1] = z_values[:, i] + step_control.h * f(x_values[i], z_values[:, i])
        x_values[i + 1] = x_values[i] + step_control.h

    return x_values, z_values


def modified_euler_method_vector(
    f: Callable[[float, np.ndarray], np.ndarray],
    x0: float,
    z0: np.ndarray,
    step_control: StepSizeControl,
) -> tuple[np.ndarray, np.ndarray]:
    """Implements the Modified Euler (Heun) method for solving vectorial ODE systems.
    Args:
        f: The function representing the ODE (dz/dx = f(x, z)).
        x0: The initial x value.
        z0: The initial z values.
    ...
    for i in range(step_control.n):
        ...
        z_i = z_values[:, i]
        k1 = f(x_i, z_i)
        k2 = f(x_i + step_control.h, z_i + step_control.h * k1)
        z_values[:, i + 1] = z_i + (step_control.h / 2) * (k1 + k2)
